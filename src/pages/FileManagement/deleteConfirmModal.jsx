...
export default function ConfirmModal({
  ...
  const iconWrap = {
    ...
    display: "grid",
    placeItems: "center",
    background: danger ? "rgba(220,38,38,0.12)" : "rgba(185,28,28,0.12)",
    border: danger
  ...
  const titleStyle = {
    fontWeight: 950,
    fontSize: 16,
    color: danger ? (C?.danger || "#dc2626") : (C?.primaryDark || "#7f1d1d"),
  };
  const desc = {
    marginTop: 4,
    fontSize: 12,
  ...
  const btnConfirm = {
    ...btnBase,
    border: `1px solid ${danger ? (C?.danger || "#dc2626") : (C?.primary || "#b91c1c")}`,
    background: danger ? (C?.danger || "#dc2626") : (C?.primary || "#b91c1c"),
    color: "#fff",
  };
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <div style={iconWrap}>
              <HiOutlineExclamationCircle
                size={22}
                color={danger ? (C?.danger || "#dc2626") : (C?.primary || "#b91c1c")}
              />
            </div>

            <div>
              <div style={titleStyle}>{title}</div>
              {!!subtitle && <div style={desc}>{subtitle}</div>}
            </div>
          </div>

          <button style={xBtn} onClick={() => !busy && onCancel?.()} aria-label="Close">
            <HiOutlineX />
          </button>
        </div>

        <div style={body}>
          <div style={note}>{message}</div>
        </div>

        <div style={foot}>
          <button style={btnCancel} onClick={() => !busy && onCancel?.()} disabled={busy}>
            {cancelText}
          </button>
          <button style={btnConfirm} onClick={() => !busy && onConfirm?.()} disabled={busy}>
